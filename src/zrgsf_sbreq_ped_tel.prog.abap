...
* Declarações de variáveis locais para fazer tratamento das DATAS. *
  ...
  lv_data_ref = sy-datum - 30.           "lv_data_ref IGUAL A sy-datum - 30 (data de referência: 30 dias atrás).
  ...
  s_aedat = VALUE #( sign   = 'I'        "SELEÇÃO s_aedat IGUAL A VALOR ( SINAL IGUAL A 'I' - Incluir
                     option = 'BT'       "OPÇÃO IGUAL A 'BT' - Between (entre dois valores)
                     low    = lv_low     "Primeiro dia do mês
                     high   = lv_high ). "Último dia do mês.

  APPEND s_aedat.                        "ANEXAR SELEÇÃO s_aedat (inclui essa linha no range s_aedat).
